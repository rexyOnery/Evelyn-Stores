...
// expose init function so dynamic frameworks (Blazor) can initialize after render
window.initPosCategoryCarousel = function(retries = 6, delay = 250) {
    // jQuery is case-sensitive: check window.jQuery or window.$
    if (typeof window.jQuery === 'undefined' || typeof $.fn === 'undefined') {
        if (retries > 0) {
            setTimeout(function() { window.initPosCategoryCarousel(retries - 1, delay); }, delay);
            return;
        }
        console && console.warn && console.warn('jQuery not available to initialize pos-category carousel');
        return;
    }

    // ensure owl plugin is available
    if (typeof $.fn.owlCarousel !== 'function') {
        if (retries > 0) {
            setTimeout(function() { window.initPosCategoryCarousel(retries - 1, delay); }, delay);
            return;
        }
        console && console.warn && console.warn('owlCarousel plugin not loaded');
        return;
    }

    try {
        $('.pos-category').each(function() {
            var $el = $(this);
            // owl adds 'owl-loaded' when initialized
